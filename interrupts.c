...
#define TIMER1LOW 0xF6
#define TIMER1HIGH 0xC2

// ARM
char directionTravelingArm;
long int clickRotationsArm;
...
double angle_fast;
char fast_grab;
int gyro_fast_center;
int gyro_fine_center;

double adjustment;

double fastMultiplier, slowMultiplier;

int gyro; // Static so that memory is not re-allocated every time.
...
void Initialize_Interrupts(void)  {
	...
	// ARM
	directionTravelingArm = 0;
	clickRotationsArm = 0;
	
	// GYRO
	angle = 0.0f;	
	fast_grab = 0;
	gyro_fine_center = (int) Get_Analog_Value(GYROFINE);
	gyro_fast_center = (int) Get_Analog_Value(GYROFAST);
	
	
	
	fastMultiplier = 1;// DPSPERNFAST * SAMPLE_PERIOD_FAST * (2.*pi)/98.;
	slowMultiplier = 1;//DPSPERNFINE * SAMPLE_PERIOD_FINE * (2.*pi);///20.;
	...
	TRISBbits.TRISB3 = 1;		// make sure the RB3/CCP2/INT3 pin is configured as an input [108]
								//
	INTCON2bits.INT3P = 0;		// 0: interrupt 2 is low priority (leave at 0 for IFI controllers) [90]
	...
	INTCONbits.RBIF = 0;		// 0: none of the interrupt 3-6 pins has changed state (set to 0 before enabling the interrupts) [89]
								...
								//
	INTCONbits.RBIE = 1;		// 0: disable interrupts 3-6 [89]
...
{
	TMR1L = TIMER1LOW;			// least significant 8-bits of the timer 1 register (this is readable and writable)
	TMR1H = TIMER1HIGH;			// most significant 8-bits of the timer 1 register (this is readable and writable)
							//
	...
	T1CONbits.T1CKPS1 = 1;	//   0      0		1:1 prescaler (clock=10MHz/each tick=100ns)
							...
							//
	T1CONbits.T1OSCEN = 1;	// 0: timer 1 oscillator disabled (leave at 0 to allow the use of an external clock)
...
{
	//static int gyro; // Static so that memory is not re-allocated every time.
	static char highExtreme;
	static char lowExtreme;
	
	gyro = ((int) Get_Analog_Value(GYROFINE)) - gyro_fine_center;
	highExtreme = gyro > 400;
	lowExtreme = gyro < -400;
	
	if(highExtreme || lowExtreme) { // EG if out of the range of the fine gyro 
			if(fast_grab) angle -= (((int)Get_Analog_Value(GYROFAST)) - gyro_fast_center) * fastMultiplier;
			Pwm2_red = 1;
			Pwm1_red = 0;
	}
	else {
			if( gyro > 4 || gyro < -4) angle -= gyro * (1.0/adjustment);
			Pwm1_red = 1;
	}
...
void Timer_1_Int_Handler(void)
{
	static unsigned int gyro;
	static int gyro_fine_recenter;
	static char highExtreme;
	static char lowExtreme;
	
	gyro = Get_Analog_Value(GYROFINE);
	gyro_fine_recenter = (((int) Get_Analog_Value(GYROFINE)) - gyro_fine_center);
	highExtreme = gyro_fine_recenter > 400;
	lowExtreme = gyro_fine_recenter < -400;
	
	if(highExtreme || lowExtreme) { /* EG if out of the range of the fine gyro *
			if(fast_grab) {
				gyro = Get_Analog_Value(GYROFAST);
				angular_rate = (gyro - gyro_fast_center) * DPSPERNFAST;
				angle_fast = ((angular_rate * SAMPLE_PERIOD_FAST) * (2.*pi)/98.);
				angle -= angle_fast;
			}
			Pwm1_green = 1;
			Pwm1_red = 0;
			...
			if( (gyro - gyro_fine_center) > 4 || (gyro - gyro_fine_center) < -4) {
				angular_rate = (gyro - gyro_fine_center) * DPSPERNFINE;
				angle_fine = ((angular_rate * SAMPLE_PERIOD_FINE) * (2.*pi)/27.2);
				angle -= angle_fine;
			}
			Pwm1_green = 0;
			Pwm1_red = 1;
...
//	gyro = Get_Analog_Value(GYROFINE);
	...
	gyro_fine_recenter = (Get_Analog_Value(GYROFINE) - gyro_fine_center);
	highExtreme = gyro_fine_recenter > 400;
	lowExtreme = gyro_fine_recenter < -400;
	
//	angle_fast = angle_fine = 0;
	
	if(highExtreme || lowExtreme) { /* EG if out of the range of the fine gyro *
			if(fast_grab) {
				angular_rate = (((double) Get_Analog_Value(GYROFAST)) - gyro_fast_center) * DPSPERNFAST;
				angle_fast =  ((angular_rate * SAMPLE_PERIOD_FAST) * (2.*pi)/98.);
				angle += angle_fast;
			}
			//Pwm1_red = 1;
			...
			if( ( Get_Analog_Value(GYROFINE) - gyro_fine_center) > 4 ||
				( Get_Analog_Value(GYROFINE) - gyro_fine_center) < -4) {
				angular_rate = (((double) Get_Analog_Value(GYROFINE)) - gyro_fine_center) * DPSPERNFINE;
				angle += /*angle_fine =* ((angular_rate * SAMPLE_PERIOD_FINE) * (2.*pi)/27.2);
//				Pwm1_red = 0;
//				angle += angle_fine;
			}
	}
...
/*void Timer_1_Int_Handler(void)
{
	int gyro_fine_recenter = (((int) Get_Analog_Value(GYROFINE)) - gyro_fine_center);
	char highExtreme = gyro_fine_recenter > 400;
	char lowExtreme = gyro_fine_recenter < -400;	
	angle_fast = angle_fine = 0;
	...
	if(highExtreme || lowExtreme) { // EG if out of the range of the fine gyro 
		...
		//	User_Byte1 = 2;
			if( ( Get_Analog_Value(GYROFINE) - gyro_fine_center) > 4 ||
				( Get_Analog_Value(GYROFINE) - gyro_fine_center) < -4) {
			//	angle+= 1;//(double)(Get_Analog_Value(GYROFINE) - gyro_fine_center);
				angular_rate = ( Get_Analog_Value(GYROFINE) - gyro_fine_center);// * DPSPERNFINE;
				angle_fine = ((angular_rate));// * SAMPLE_PERIOD_FINE) * (2.*pi)/68.);
				//Pwm1_red = 0;
				angle += angle_fine;
			//	User_Byte1 = 1;
			}
...
{
//	User_Byte2 = 3;
	if(ARMENC2 == INPUTTRUE){
//		User_Byte3 = 1;
		clickRotationsArm++;
		directionTravelingArm = BACKWARD;
	}else{
//		User_Byte3 = 2;
		clickRotationsArm--;
		directionTravelingArm = FORWARD;
	}
}
