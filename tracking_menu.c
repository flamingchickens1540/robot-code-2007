...
unsigned char Tracking_Menu(void)
{
	...
	switch(menu_state)
	{
		...
		case TM_MENU_HANDLER:
			...
			if(terminal_char >= 'a' && terminal_char <= 'r')
			{
		...
		case TM_GET_DEFAULT_VALUES:
			...
			Temp_Tracking_Config_Data.Confidence_Threshold = CONFIDENCE_THRESHOLD_DEFAULT;
...
void Tracking_Menu_Print(void)
{
	printf("\r\n");
	printf("\r\n");
	printf("[a] Change Pan Min PWM = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Pan_Min_PWM);
	printf("[b] Change Pan Center PWM = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Pan_Center_PWM);
	printf("[c] Change Pan Max PWM = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Pan_Max_PWM);
	printf("[d] Change Pan Gain = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Pan_Gain);
	printf("[e] Change Pan Allowable Error = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Pan_Allowable_Error);
	printf("[f] Change Pan Rotation Sign = %+d\r\n",(int)Temp_Tracking_Config_Data.Pan_Rotation_Sign);
	printf("[g] Change Pan Search Step Size = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Pan_Search_Step_Size);
	printf("[h] Change Pan Target Pixel = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Pan_Target_Pixel);
	printf("[i] Change Tilt Min PWM = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Tilt_Min_PWM);
	printf("[j] Change Tilt Center PWM = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Tilt_Center_PWM);
	printf("[k] Change Tilt Max PWM = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Tilt_Max_PWM);
	printf("[l] Change Tilt Gain = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Tilt_Gain);
	printf("[m] Change Tilt Allowable Error = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Tilt_Allowable_Error);
	printf("[n] Change Tilt Rotation Sign = %+d\r\n",(int)Temp_Tracking_Config_Data.Tilt_Rotation_Sign);
	printf("[o] Change Tilt Search Step Size = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Tilt_Search_Step_Size);
	printf("[p] Change Tilt Target Pixel = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Tilt_Target_Pixel);
	printf("[q] Change Search Delay = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Search_Delay);
	printf("[r] Change Confidence Threshold = %u\r\n",(unsigned int)Temp_Tracking_Config_Data.Confidence_Threshold);
	printf("\r\n");
